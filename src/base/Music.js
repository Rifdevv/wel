...
class MusicPlayer {
  ...
  async getLyrics(name) {
    try {
      ...
      if (!result.full_title || !result.lyrics || result.lyrics.length == 0)
        return false;
      return {
        title: result.title,
  ...
  skip(guild) {
    ...
    setTimeout(() => {
      if (revertLoopToTrack && this._queue.get(guild.id))
        this._queue.get(guild.id).loop = 2;
    }, 1000);
    return true;
  }
  ...
  async handleVideo(video, message, voiceChannel, playlist = false) {
    ...
    if (!queue) {
      let joinMessage = await message.channel.createMessage(
        `${this.client.emojis.headphones} | Masuk **${
          this.client.getChannel(voiceChannel.channelID).name
        }**.`
      );
      let queueConstruct = this.constructQueue(message.channel, voiceChannel);
      queueConstruct.connection = await this.client.joinVoiceChannel(
      ...
      await joinMessage
        .edit(
          `${this.client.emojis.tick} | Masuk ke **${
            this.client.getChannel(voiceChannel.channelID).name
          }**.`
        )
        .then(r => r.delete(6000));
      ...
      if (playlist == false)
        message.channel.createMessage(
          `${this.client.emojis.tick} | Ditambahkan **${song.title}** ke dalam antrian.`
        );
    }
  }
  ...
  play(guild, song) {
    ...
    if (!song) {
      ...
      this.client.leaveVoiceChannel(
        serverQueue.voiceChannel.channelID
          ? serverQueue.voiceChannel.channelID
          : serverQueue.voiceChannel
      );
      return serverQueue.textChannel.createMessage(
        `${this.client.emojis.music} | Musik telah Berakhir.`
      );
    }
    serverQueue.connection.play(
      ...
      {
        encoderArgs: serverQueue.bassboost
          ? ["-af", "equalizer=f=40:width_type=h:width=50:g=10"]
          : []
      }
    );
    serverQueue.connection.once("end", () => {
      setTimeout(() => {
        const shifted =
          serverQueue.loop == 2 ? false : serverQueue.songs.shift();
    ...
    serverQueue.textChannel.createMessage(
      `${this.client.emojis.music} | Memutar **${song.title}**`
    );
  }
}
