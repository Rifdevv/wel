...
class lyrics extends Command {
    ...
    constructor(client) {
        super(client, {
            name: "lyrics",
            description: "Shows the Lirik of the given song name or of the current one.",
            usage: "lyrics",
            enabled: true,
            aliases: ["ly"]
        })
    ...
    async run(message, args, Eris) {
        ...
        if(!name) return message.channel.createMessage(`${this.client.emojis.cross} | Anda harus menentukan Nama Lagu untuk menggunakan \`${this.help.name}\` command.`);
        const searchingMsg = await message.channel.createMessage(`${this.client.emojis.search} | Mencari Lirik untuk \`${name}\``);
        this.lyrics = await this.client.music.getLyrics(name);
        if(!this.lyrics) return searchingMsg.edit(`${this.client.emojis.cross} | Tidak ditemukan Lirik untuk \`${name}\`.`);
