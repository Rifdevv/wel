...
class search extends Command {
    ...
    async run(message, args, Eris) {
      ...
      const embed = {
            ...
            footer: {
                text: `Song Selection (1 - ${videos.length}) | .`
            }
      };
      ...
      if(responses.length && responses[0].content.toLowerCase() == "cancel") {
          await msg.delete();
          message.channel.createMessage(`${this.client.emojis.cross} | Pencarian video yang dibatalkan.`);
          return;
          ...
          await msg.delete();
          message.channel.createMessage(`${this.client.emojis.cross} | Nilai salah atau tidak ada Dimasukkan, pencarian video dibatalkan.`);
          return;
      }
		}
