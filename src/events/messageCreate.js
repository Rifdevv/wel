module.exports = class {
    ...
    async run(message) {
        ...
        if(!message.channel.permissionsOf(this.client.user.id).has("allText")) {
            this.client.log(`Tidak Ada Izin Suara di ${message.channel.guild.id}`);
            return;
        }
        if(!message.channel.permissionsOf(this.client.user.id).has("allVoice")) {
            this.client.createMessage(message.channel.id, `${this.client.emojis.cross} | Tanpa Izin Suara!`);
            return;
        }
